...
function text(value) {
  ...
  if (typeof value === "object") {
    ...
    if (value["__text"] != null) {
      return String(value["__text"]);
    }
  }
...
function lastToken(raw, separators = ["/", "#"]) {
  if (!raw) {
    return "";
  }
...
function normalizeWindUnit(rawUnit) {
  ...
  if (unit === "[kn_i]" || unit === "kt" || unit === "knot" || unit === "kn") {
    return "KT";
  }
  return rawUnit ? String(rawUnit) : "KT";
}

function parseWeatherCode(rawCode) {
  if (!rawCode) {
    return null;
  }
  ...
  for (let i = 0; i < cursor.length; i += 2) {
    ...
    if (chunk.length === 2 && validPhenomena.has(chunk)) {
      phenomena.push(chunk);
    }
  }
...
function resolveWeatherIconKey(weather) {
  ...
  if (weather.descriptor) {
    ...
    if (joined && knownIconKeys.has(joined)) {
      return joined;
    }
    if (knownIconKeys.has(weather.descriptor)) {
      return weather.descriptor;
    }
  }

  if (Array.isArray(weather.phenomena) && weather.phenomena.length > 0) {
    const first = weather.phenomena[0];
    if (knownIconKeys.has(first)) {
      return first;
    }
  }
...
function pickPrimaryWeatherIcon(weatherList) {
...
function formatCloudBase(baseFt) {
...
function parseCloudLayer(layerNode) {
  ...
  if (baseNode && typeof baseNode === "object") {
    ...
    if (Number.isFinite(base) && uom === "m") {
      base = Math.round(base * 3.28084);
    }
  }
...
function resolveWindBarb(wind) {
  ...
  return {
    barb_key: String(rounded),
    rotation: wind?.variable ? 0 : wind?.direction || 0,
    pennants,
    long_barbs: longBarbs,
    short_barbs: shortBarbs
  };
}

function parseWind(windNode) {
...
function resolveDdhh(ddhh, anchor) {
  let token = String(ddhh || "").trim();
  if (/^\d{1,4}$/.test(token)) {
    token = token.padStart(4, "0");
  }

  if (!/^\d{4}$/.test(token)) {
    // already ISO-like input is passed through
  ...
  if (tooPast) {
    resolved = new Date(Date.UTC(year, month + 1, day, hour, 0, 0, 0));
  }

  return resolved.toISOString().replace(".000Z", "Z");
}

function parseYmdhmToIso(raw) {
  ...
  const min = Number(token.slice(10, 12));
...
function toMetarTempToken(v) {
  if (!Number.isFinite(v)) {
    return "//";
  }
  if (v < 0) {
    return `M${String(Math.abs(v)).padStart(2, "0")}`;
  }
