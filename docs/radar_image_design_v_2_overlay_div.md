...
### 8.2 동작
- 입력: `radarData.images`
- 기본 프레임 전환 주기: `500ms` (사용자 조절 가능)
- 이미지 없으면 `"Radar data unavailable."`
- 일시정지 상태에서는 최신 프레임으로 자동 갱신 고정
...
- 재생/일시정지 버튼:
  ...
  - 아이콘: `▶` / `❚❚`
- 재생시간 컨트롤:
  - 헤더 우측 `재생시간 {N.N}초` 칩 + `- / +` 버튼
  - 범위: `0.1초 ~ 2.0초` (`100ms` step)
- 타임라인(시커 바):
  - 사용자가 직접 프레임 인덱스를 이동 가능
  - 하단에 현재 시각(`HH:mm`) 표시
  - 시각 배치 시 range thumb 폭을 보정하여 슬라이더와 정렬
...
| 8 | 재생/일시정지 | 버튼으로 재생 상태 전환, 일시정지 시 최신 프레임 자동 반영 |
| 9 | 재생시간 조절 | `- / +`로 0.1~2.0초 범위 조절 |
| 10 | 타임라인 시커 이동 | 슬라이더 이동 시 해당 프레임/시각으로 즉시 반영 |
| 11 | 공항 선택 변경 | 오버레이 마커 위치 변경 |
