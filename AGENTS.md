...
- InteractiveMap `SIGWX_LOW` marker icons now use assets under `frontend/public/icon_sigwx/`. Phenomenon icons render without a white badge background; only meaningful sublabels remain as separate white chips. `freezing_level` uses `빙결고도.png` and is intentionally scaled larger than other SIGWX icons.
- InteractiveMap `SIGWX_LOW` no longer renders `font_line` (`fl_cold`, `fl_worm`, `fl_occl`) as normal GIS polylines. Fronts are rendered server-side into transparent PNG overlays by `backend/src/parsers/sigwx-front-overlay.js`, exposed via `/api/sigwx-low-fronts?tmfc=...`, and composited in the map with `ImageOverlay`.
- `SIGWX_LOW` front overlays are `tmfc`-aware. When the user changes SIGWX history frame, the frontend reloads the front overlay for the selected `tmfc`; if that snapshot has no `font_line`, no front overlay is shown.
