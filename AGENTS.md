...
- Mobile `/ops` now uses a dedicated layout (desktop `/ops` and `/ground` stay unchanged): bottom tabs `현재날씨 / 예보 / 지도`, swipeable tab transitions, pull-to-refresh hint/reload, and a tab-entry animation.
- Mobile `/ops` `현재날씨` always renders `WarningList` at the top of the tab, then a mobile `MetarCard` with sunrise/sunset in the header and cards for `시정/운고/바람/측풍/현재날씨/온도/일강수량/QNH`.
- Mobile `/ops` `예보` view labels are inverted from the internal modes: toggle label `카드` = mobile `table` card-list view with left proportional timeline bar, while toggle label `표` = mobile `v3` row-table view. Both mobile forecast views must preserve the same condition styling rules as desktop/mobile TAF (visibility/ceiling tint, high-wind dashed alert, special/precip weather emphasis).
- Mobile `/ops` `지도` enters on nationwide scope and must not auto-reset after the user zooms/pans; Leaflet visibility re-entry logic includes additional guardrails against invalid `LatLng` values.
