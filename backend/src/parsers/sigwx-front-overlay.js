...
async function renderSigwxFrontOverlay(sigwxLow, dataRoot, canonicalHash) {
  ...
  const shortHash = String(canonicalHash || "").slice(0, 10) || "trial";
  const filename = `fronts_${tmfc}_${shortHash}.png`;
  ...
  fs.writeFileSync(path.join(dir, "fronts_meta.json"), `${JSON.stringify(meta, null, 2)}\n`, "utf8");
  return meta;
}
